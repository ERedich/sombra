/**
 * PCR Remedies master CRUD. Each remedy belongs to exactly one cause (1:n).
 * Dialog has cascading Problem → Cause pickers (problem filters the cause list
 * for convenience; only cause_id is persisted).
 */
...
import type {
  ...
  PcrProblem,
  PcrRemedy,
} from '../pcr-problems/types'

type ListResponse = { remedies: PcrRemedy[] }
type ItemResponse = { remedy: PcrRemedy }
type ProblemsListResponse = { problems: PcrProblem[] }
type CausesListResponse = { causes: PcrCause[] }

export default function PcrRemediesAppPage() {
  ...
  const [rows, setRows] = useState<PcrRemedy[]>([])
  const [problems, setProblems] = useState<PcrProblem[]>([])
  const [causes, setCauses] = useState<PcrCause[]>([])
  ...
  const [formCauseId, setFormCauseId] = useState<string | null>(null)
  const [selected, setSelected] = useState<PcrRemedy | null>(null)
  const [search, setSearch] = useState('')
  const emDash = t('common.em_dash')

  const tableColumnDefs = useMemo((): ColumnRegistryEntry<PcrRemedy>[] => {
    return [
      { field: 'key', headerKey: 'common.col_key', sortable: true },
      { field: 'name', headerKey: 'common.col_name', sortable: true },
      {
        field: 'cause_name',
        headerKey: 'pcr.col_cause',
        sortable: true,
        body: (row) => `${row.cause_key} ${emDash} ${row.cause_name}`,
      },
      {
        field: 'problem_name',
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        r.cause_key.toLowerCase().includes(q) ||
        r.cause_name.toLowerCase().includes(q) ||
  ...
  const tw = useTableWizard<PcrRemedy>({
    appPath: '/pcr-remedies',
  ...
  const loadRows = useCallback(async () => {
    setLoading(true)
    try {
      const [remedies, problemsData, causesData] = await Promise.all([
        apiJson<ListResponse>('/api/pcr-remedies'),
        apiJson<ProblemsListResponse>('/api/pcr-problems'),
        apiJson<CausesListResponse>('/api/pcr-causes'),
      ])
      setRows(remedies.remedies ?? [])
  ...
  const causeOptions = useMemo(() => {
    const base = formProblemId
      ? causes.filter((c) => c.problem_id === formProblemId)
      : causes
    return base.map((c) => ({
      value: c.id,
      label: `${c.key} ${emDash} ${c.name}`,
    }))
  }, [causes, formProblemId, emDash])
  ...
  function openEdit(row: PcrRemedy) {
    setEditingId(row.id)
    setFormKey(row.key)
    setFormName(row.name)
    setFormDescription(row.description ?? '')
    setFormProblemId(row.problem_id)
    setFormCauseId(row.cause_id)
    setDialogOpen(true)
  }

  function onProblemChange(next: string | null) {
    setFormProblemId(next)
    if (next && formCauseId) {
      const c = causes.find((x) => x.id === formCauseId)
      if (!c || c.problem_id !== next) setFormCauseId(null)
    }
  }
  ...
  async function saveRow() {
    ...
    if (!formCauseId) {
      showError(t('pcr.err_cause_required'))
      return
    }
    setSaving(true)
    try {
      const body = {
        ...
        cause_id: formCauseId,
      }
      if (editingId) {
        const data = await apiJson<ItemResponse>(
          `/api/pcr-remedies/${editingId}`,
          { method: 'PATCH', body: JSON.stringify(body) },
        )
        setRows((prev) =>
          prev.map((c) => (c.id === editingId ? data.remedy : c)),
        )
        setSelected((cur) => (cur?.id === editingId ? data.remedy : cur))
        showSuccess(t('pcr.updated'))
      } else {
        const data = await apiJson<ItemResponse>('/api/pcr-remedies', {
          method: 'POST',
          body: JSON.stringify(body),
        })
        setRows((prev) =>
          [...prev, data.remedy].sort((a, b) =>
  ...
  function confirmDeleteRow(row: PcrRemedy) {
    confirmDialog({
  ...
  async function deleteRow(id: string) {
    try {
      await apiJson<undefined>(`/api/pcr-remedies/${id}`, { method: 'DELETE' })
  ...
  const crudContextMenuItems = [
    buildAskKiraMenuItem(t, {
      ...
      getDraft: () =>
        selected
          ? formatKiraRowDraft(t('pcr.remedies.title'), {
              id: selected.id,
              key: selected.key,
              name: selected.name,
            })
    ...
    ...buildCrudContextMenuModel(
      ...
      t,
      {
        ...
        auditHistory: {
          ...
          onNavigate: () =>
            navigate(
              `/audit-log?resource_type=pcr_remedy&resource_id=${encodeURIComponent(auditResourceIdForMenu)}`,
            ),
        },
      },
    ),
    ...
    <div className="app-card-hero flex align-items-start justify-content-between gap-3 p-4 md:p-5 w-full flex-wrap">
      <div className="flex align-items-start gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0 pt-0">
          <h1 className="app-card-hero-title">{t('pcr.remedies.title')}</h1>
          <p className="app-card-hero-desc">{t('pcr.remedies.subtitle')}</p>
        </div>
      </div>
        ...
        >
          <div className="px-1 md:px-2">
            ...
            <DataTable
              ...
              selection={selected}
              onSelectionChange={(e) =>
                setSelected(e.value as PcrRemedy | null)
              }
      ...
      >
        <div className="flex flex-column gap-3 pt-2">
          <div className="flex flex-column gap-2">
            <label htmlFor="rem-problem" className="text-sm font-medium">
            ...
            <Dropdown
              ...
              onChange={(e) =>
                onProblemChange(typeof e.value === 'string' ? e.value : null)
              }
              optionLabel="label"
              optionValue="value"
              filter
          ...
          <div className="flex flex-column gap-2">
            <label htmlFor="rem-cause" className="text-sm font-medium">
              {t('pcr.field_cause_id')}
            </label>
            <Dropdown
              inputId="rem-cause"
              value={formCauseId}
              ...
              onChange={(e) =>
                setFormCauseId(typeof e.value === 'string' ? e.value : null)
              }
              optionLabel="label"
              optionValue="value"
              filter
              showClear
              placeholder={t('pcr.field_cause_id')}
              className="w-full"
              disabled={saving}
            />
          </div>
          <div className="flex flex-column gap-2">
            <label htmlFor="rem-key" className="text-sm font-medium">
              {t('common.col_key')}
            </label>
            <InputText
              id="rem-key"
          ...
          <div className="flex flex-column gap-2">
            <label htmlFor="rem-name" className="text-sm font-medium">
              {t('common.col_name')}
            </label>
            <InputText
              id="rem-name"
