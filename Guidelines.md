...
- **Sticky tab nav (MW):** The tab **nav bar** must stay pinned at the top of the dialog’s scrolling area — it **does not scroll out of view** when the user scrolls the tab body (or the dialog content when that is the active scroll container). This is enforced by **`app-modal-tabview`** in [`frontend/src/App.css`](frontend/src/App.css) via `position: sticky; top: 0;` on `.p-tabview-nav-container` with a solid `var(--surface-overlay)` background so content scrolling beneath is covered; the ink bar animates beneath the same sticky band. When tab headers **overflow** horizontally, the nav strip scrolls (`overflow-x: auto` + `flex-wrap: nowrap; min-width: max-content;` on `.p-tabview-nav`); use the scrollbar or horizontal trackpad scroll — do **not** wrap tab headers to multiple rows.
