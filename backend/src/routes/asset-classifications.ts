...
type AssetClassificationTableRow = {
  id: string
  site_id: string
  key: string
  name: string
...
type AssetClassificationRow = AssetClassificationTableRow & {
  site_key: string
  site_name: string
  site_colour: string
...
async function fetchAssetClassificationWithJoins(
  client: PoolClient,
  id: string,
): Promise<AssetClassificationRow | undefined> {
  const r = await client.query<AssetClassificationRow>(
    `SELECT ac.id, ac.site_id, ac.key, ac.name, ac.created_at, ac.updated_at,
     ...
     INNER JOIN sites st ON st.id = ac.site_id
     LEFT JOIN users cb ON cb.id = ac.created_by
     LEFT JOIN users ub ON ub.id = ac.updated_by
     WHERE ac.id = $1`,
    [id],
  )
  return r.rows[0]
...
function rowToAuditRecord(
  row: AssetClassificationTableRow,
...
router.get('/', async (req, res) => {
  ...
  if (auth.role === 'admin') {
    const r = await pool.query<AssetClassificationRow>(
      `${LIST_SQL} ORDER BY st.name ASC, st.key ASC, ac.name ASC, ac.key ASC`,
    )
    res.json({ asset_classifications: r.rows })
    return
  }
  const scope = await loadUserSiteScope(pool, auth.id, auth.role)
  const allowed = accessibleSiteIds(scope)
  if (allowed === null || allowed.length === 0) {
    res.json({ asset_classifications: [] })
    return
  }
  const r = await pool.query<AssetClassificationRow>(
    `${LIST_SQL} WHERE ac.site_id = ANY($1::uuid[])
     ORDER BY st.name ASC, st.key ASC, ac.name ASC, ac.key ASC`,
    [allowed],
  )
  res.json({ asset_classifications: r.rows })
})
...
router.get('/:id', async (req, res) => {
  ...
  if (!UUID_RE.test(id)) {
    res.status(400).json({ error: 'Invalid asset classification id.' })
    return
  }
  const auth = req.authUser!
  const scope = await loadUserSiteScope(pool, auth.id, auth.role)
  const r = await pool.query<AssetClassificationRow>(
    `${LIST_SQL} WHERE ac.id = $1`,
    [id],
  )
  const row = r.rows[0]
  if (!row || !canAccessSite(scope, row.site_id)) {
    res.status(404).json({ error: 'Asset classification not found.' })
    return
  }
  res.json({ asset_classification: row })
})
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const r = await client.query<{ id: string }>(
      `INSERT INTO asset_classifications (key, name, site_id, created_by)
       VALUES ($1, $2, $3, $4)
       RETURNING id`,
    ...
    const tableRow = await client.query<AssetClassificationTableRow>(
      `SELECT id, site_id, key, name, created_at, updated_at, created_by, updated_by
       FROM asset_classifications WHERE id = $1`,
      [insertedId],
    )
    ...
    await writeAudit(client, {
      ...
      actorName: auth.name,
      operation: 'create',
      resourceType: 'asset_classification',
    ...
    const assetClassification = await fetchAssetClassificationWithJoins(
      client,
      insertedId,
    )
    await client.query('COMMIT')
    res.status(201).json({ asset_classification: assetClassification! })
  } catch (e) {
    await client.query('ROLLBACK')
    if (isUniqueViolation(e)) {
      res.status(409).json({
        error:
          'An asset classification with this key already exists at this site.',
      })
      return
    }
    throw e
...
router.patch('/:id', async (req, res) => {
  ...
  const sql = `UPDATE asset_classifications SET ${updates.join(', ')}
               WHERE id = $${n}
  ...
  try {
    await client.query('BEGIN')
    const prev = await client.query<AssetClassificationTableRow>(
      `SELECT id, site_id, key, name, created_at, updated_at, created_by, updated_by
    ...
    const r = await client.query<AssetClassificationTableRow>(sql, values)
    ...
    await client.query('COMMIT')
    res.json({ asset_classification: assetClassification! })
  } catch (e) {
    await client.query('ROLLBACK')
    if (isUniqueViolation(e)) {
      res.status(409).json({
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM asset_classifications WHERE id = $1`, [id])
