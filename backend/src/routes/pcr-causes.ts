...
type PcrCauseRow = PcrCauseTableRow & {
  site_key: string
  site_name: string
  site_colour: string
  problem_key: string
...
async function fetchCauseWithJoins(
  client: PoolClient,
  id: string,
): Promise<PcrCauseRow | undefined> {
  const r = await client.query<PcrCauseRow>(
    `SELECT c.id, c.site_id, c.problem_id, c.key, c.name, c.description,
            c.created_at, c.updated_at, c.created_by, c.updated_by,
            st.key AS site_key, st.name AS site_name, st.colour AS site_colour,
            p.key AS problem_key, p.name AS problem_name,
...
function rowToAuditRecord(row: PcrCauseTableRow): Record<string, unknown> {
...
router.get('/', async (req, res) => {
  const orderBy = `ORDER BY st.name ASC, st.key ASC, p.key ASC, c.key ASC`
  ...
  if (auth.role !== 'admin') {
    ...
    params.push(allowed)
    filters.push(`c.site_id = ANY($${params.length}::uuid[])`)
  }
  if (problemId) {
  ...
  const r = await pool.query<PcrCauseRow>(
    `${LIST_SQL} ${where} ${orderBy}`,
    params,
  )
  res.json({ causes: r.rows })
})
...
router.get('/:id', async (req, res) => {
  ...
  const auth = req.authUser!
  const userScope = await loadUserSiteScope(pool, auth.id, auth.role)
  const r = await pool.query<PcrCauseRow>(`${LIST_SQL} WHERE c.id = $1`, [id])
  const row = r.rows[0]
...
router.post('/', async (req, res) => {
  ...
  const description =
    typeof req.body?.description === 'string'
      ...
      : null
  const problemId =
    typeof req.body?.problem_id === 'string' ? req.body.problem_id.trim() : ''
  ...
  if (!UUID_RE.test(problemId)) {
    res.status(400).json({ error: 'A valid problem_id is required.' })
    return
  }
  ...
  try {
    ...
    if (
      ...
      res.status(400).json({ error: 'Parent problem not found for this site.' })
      return
    }

    const r = await client.query<{ id: string }>(
      `INSERT INTO pcr_causes (key, name, description, site_id, problem_id, created_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id`,
      [key, name, description, siteId, problemId, auth.id],
    )
    ...
    const tableRow = await client.query<PcrCauseTableRow>(
      `SELECT id, site_id, problem_id, key, name, description, created_at, updated_at, created_by, updated_by
       FROM pcr_causes WHERE id = $1`,
      [insertedId],
    )
    ...
    const cause = await fetchCauseWithJoins(client, insertedId)
    await client.query('COMMIT')
    res.status(201).json({ cause: cause! })
  } catch (e) {
    await client.query('ROLLBACK')
    if (isUniqueViolation(e)) {
      res.status(409).json({
        error: 'A cause with this key already exists for this site.',
      })
      return
    }
    throw e
...
router.patch('/:id', async (req, res) => {
  ...
  if (req.body?.problem_id !== undefined) {
    const pid =
      typeof req.body.problem_id === 'string' ? req.body.problem_id.trim() : ''
    if (!UUID_RE.test(pid)) {
      res.status(400).json({ error: 'Invalid problem_id.' })
      return
    }
    updates.push(`problem_id = $${n++}`)
    values.push(pid)
  }
  ...
  const sql = `UPDATE pcr_causes SET ${updates.join(', ')}
               WHERE id = $${n}
               RETURNING id, site_id, problem_id, key, name, description, created_at, updated_at, created_by, updated_by`
  ...
  try {
    await client.query('BEGIN')
    const prev = await client.query<PcrCauseTableRow>(
      `SELECT id, site_id, problem_id, key, name, description, created_at, updated_at, created_by, updated_by
       FROM pcr_causes
       WHERE id = $1
       FOR UPDATE`,
    ...
    const r = await client.query<PcrCauseTableRow>(sql, values)
    ...
    const cause = await fetchCauseWithJoins(client, afterTable.id)
    await client.query('COMMIT')
    ...
    if (isForeignKeyViolation(e)) {
      res.status(400).json({ error: 'Referenced problem does not exist.' })
      return
    }
    throw e
  } finally {
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM pcr_causes WHERE id = $1`, [id])
    ...
    if (isForeignKeyViolation(e)) {
      res.status(409).json({
        error:
          'This cause is referenced by remedies and cannot be deleted.',
      })
      return
    }
    throw e
