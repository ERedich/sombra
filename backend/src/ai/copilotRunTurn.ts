...
function buildSystemPrompt(args: {
  ...
  return [
    ...
    `**Shifts & capacity planning:** use \`list_shift_definitions\` for site shift templates (times, weekdays). Use \`get_scheduling_snapshot\` with \`date_from\` / \`date_to\` (YYYY-MM-DD, inclusive) for work orders with plan windows in that range, shift assignments, capacity allocations, and used hours per employee per day — max ${COPILOT_SCHEDULING_MAX_RANGE_DAYS} calendar days per call. Use \`analyze_scheduling_issues\` for the same window when the user asks to **run a check**, find **bottlenecks**, **collisions**, **overload**, or **scheduling risks**; it returns structured issues (read-only). Prefer narrowing with \`workgroup_id\` when the user cares about one team. To **set or clear** a single capacity allocation (employee + day + hours on a WO) from chat, use \`prepare_set_capacity_allocation\` after validating with \`get_work_order_details\` / \`find_assignable_employees\` as needed. To **assign an employee to a shift template for one calendar day** (Schichtzuweisung — same model as the shift planner), use \`prepare_create_shift_assignment\` with ids from \`list_shift_definitions\` and site employees; the weekday of \`assignment_date\` must match the shift’s \`available_weekdays\`. The user confirms in Kira. Offer \`request_client_navigation\` to apps \`capacity_planner\`, \`shift_planner\`, or \`shifts\` when bulk UI work is still needed.`,
    ...
    'You never create or update database records yourself. Use tools to read data and to register validated create/update payloads (including `prepare_set_capacity_allocation` for one Kapazitätsplaner cell and `prepare_create_shift_assignment` for one shift assignment); the user must confirm writes in the app.',
